...
db.query(`CREATE TABLE IF NOT EXISTS guilds(id INTEGER PRIMARY KEY AUTOINCREMENT, guildID TEXT, channelID TEXT, message TEXT, elections BOOLEAN)`)

export function setGuild(id: string) {
    db.query(`INSERT INTO guilds (guildID, channelID, message, elections) VALUES (?, ?, ?, ?)`, [id, 'none', 'as a member of the guild, it is your duty to vote. Please do so using the command /vote', false])
}
